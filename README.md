...
warm copper contour-line accent, drawn from the benchmark-disk mark) — and then
...
| `public/favicon.svg` · `public/lentago-mark-square.svg` · `public/banner.svg` | The benchmark-disk brand assets (favicon, square mark, README/og banner). |
| `Dockerfile` / `nginx.conf` | Packages the built `dist/` into an `nginx` container on port `8080` with a `/health` endpoint for the ALB. |
| `.github/workflows/deploy.yml` | Build → ECR → ECS rollout via OIDC. Manual-dispatch only until the foundry infra for Lentago is provisioned (see below). |
...
> **Deploy wiring is pending.** `deploy.yml` is parameterised for ECR/ECS
> `foundry-dev-lentago` but runs on **manual dispatch only** — the ECR repo, ECS
> service, and the OIDC trust for *this* repo must first be provisioned by
> `foundry-platform-demo` (`modules/site`), mirroring the pitzilabs-dev wiring.
> Until then the site builds and containerises locally but does not auto-deploy.
